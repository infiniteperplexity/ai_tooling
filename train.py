...
import time
# Should I investigate torch.backends.cudnn.benchmark as an optimization?  Supposedly good when input sizes are constant.
...
class MetricsLogger():
    ...
    def log_metrics(self, mode, step): # everything should be back on CPU at this point.
        ...
        row["step"] = step
        previous_row = self.get_latest_row(mode = mode) # this is such an obnoxious way to do this, is there really nothing better
        previous_step = 0 if previous_row is None or self.epoch != previous_row["epoch"] else previous_row["step"] 
        row["steps"] = step - previous_step
        current_timestamp = time.time()
        row["seconds"] = current_timestamp - self.logging_timestamp
        if len(self.log) == 0:
            row["total_seconds"] = row["seconds"]
        else:
            row["total_seconds"] = self.log[-1]["total_seconds"] + row["seconds"]
    ...
    def get_latest_row(self, mode = "train"):
...
class TimedStoppingCallback(TrainerCallback):
    ...
    def on_step_end(self, trainer):
        ...
        accumulated_time = 0 if log is None else log["total_seconds"]
    ...
    def on_after_log(self, trainer):
        if not self.on_after_log_:
            return
        log = trainer.logger.get_latest_train_row()
        if log is not None and log["total_seconds"] >= self.max_seconds:
            trainer.state.should_training_stop = True
            trainer.state.should_eval = True
            print("training ended by early stopping callback")
